...
plctargets = dict([(name, {"xsd":path.join(_base_path, name, "XSD"),
...
toolchains = {"cloud":  path.join(_base_path, "XSD_toolchain_cloud")}

def GetBuilder(targetname):
    return plctargets[targetname]["class"]()
...
def GetTargetChoices():
    ...
    # Get all xsd plctargets
    for targetname,nfo in plctargets.iteritems():
...
def GetTargetCode(targetname):
    codedesc = plctargets[targetname]["code"]
