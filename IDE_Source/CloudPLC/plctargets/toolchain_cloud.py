...
plc_rt_dir    = os.path.join(os.path.join(base_dir, "CloudPLC"), "src")
if (os.name == 'posix' and not os.path.isfile(plc_rt_dir)):
    plc_rt_dir = os.environ["HOME"]+"/CloudPLC/RTE/src"

class toolchain_cloud(toolchain_gcc):
    def __init__(self, CTRInstance):
        self.dev_family       = "NO_DEVICE"
        self.load_addr        = "0"
        self.runtime_addr     = "0"
        self.base_flags       = ["-mthumb",   "-g3", "-w"]
    ...
    def build(self):
	    ...
	    self.CTRInstance.logger.write("   [OBJCOPY]  " + self.exe +" -> " + self.exe + ".bin\n")


	    #objcpy = [self.toolchain_prefix + "objcopy",  "-O", "ihex", self.exe_path, self.exe_path + ".hex"]
	    objcpy = [self.toolchain_prefix + "objcopy",  "-O", "binary", self.exe_path, self.exe_path + ".bin"]
